...
import AcpModelSelector from "./AcpModelSelector.vue";
import AgentScopeSelect from "./AgentScopeSelect.vue";
  ...
  <template v-if="agent.routeToAcp">
    ...
    <template v-else>
      ...
      <span
        ...
        title="当前模型不支持思考强度；切换到支持该能力的模型（如 opencode/gpt-5-codex）后会出现选择器"
      ...
      <span
        v-if="remembered"
        title="模型 / 思考强度已按当前工作区记住，切回该工作区会自动恢复"
        class="flex h-6 shrink-0 cursor-default items-center gap-1 rounded-full border border-line bg-panel-2 px-2.5 text-[11px] text-dim2"
      >
