...
<template>
  <div class="flex size-full min-h-0 flex-col overflow-hidden" data-testid="file-tree">
    ...
    <div class="flex shrink-0 items-center gap-1.5 border-b border-line px-3 py-1.5">
      <span class="min-w-0 flex-1 truncate font-mono text-[10.5px] text-dim2" :title="rootTitle">
      ...
      <button
        ...
        :title="tree.bound ? '换绑工作区文件夹：既有会话文件一并搬到新目录' : '绑定工作区文件夹：文件树与产物都以该文件夹为根'"
        :disabled="binding"
        @click="bind()"
      >
    ...
    <div v-else class="min-h-0 flex-1 overflow-auto py-1">
      ...
      >
        ...
        <Icon v-if="row.node.kind === 'directory'" name="folder" :size="12" class="shrink-0 text-dim2" />
