...
/** 「更多」下拉：点条带外任意处收起（挂在 window 捕获期，覆盖不冒泡的合成事件）。 */
function onWindowPointerDown(event: PointerEvent): void {
  if (!overflowOpen.value) return;
  const target = event.target as Node | null;
  if (overflowRoot.value?.contains(target)) return;
  overflowOpen.value = false;
}
  ...
  >
    <div class="flex h-full flex-col">
      ...
      <div class="flex h-[34px] shrink-0 items-center gap-1 border-b border-line px-3">
        <button
          v-for="contribution in partitioned.pinned"
          :key="contribution.id"
          type="button"
        ...
        <div class="ms-auto flex shrink-0 items-center gap-0.5">
          <!-- 低频面板收进「更多」，不占常驻标签位 -->
          <div v-if="partitioned.overflow.length" ref="overflowRoot" class="relative">
            <button
              type="button"
            ...
            <div
              v-if="overflowOpen"
              data-testid="activity-overflow-menu"
              class="absolute bottom-full right-0 z-30 mb-1 flex min-w-[120px] shrink-0 flex-col gap-0.5 rounded-[8px] border border-line-2 bg-panel p-1"
              role="menu"
              aria-label="更多面板"
            >
              <button
                ...
                class="flex h-6 cursor-pointer items-center rounded-[5px] px-2 text-left text-[11.5px] text-dim transition-colors hover:bg-panel-2 hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-cyan"
                @click="activate(contribution.id)"
              >
                {{ contribution.title }}
              </button>
