...
afterEach(() => {
  // 提示内容 Portal 到 body，清掉免得串到下一个用例。
...
describe("LayoutModeSwitch", () => {
  ...
  it("提示带模式名与快捷键序号（原生 title 已换成 Hint）", async () => {
    const wrapper = mountSwitch();
    const button = buttonOf(wrapper, "split");

    // 换成 Hint 之后按钮上不再有 title（这正是迁移目的：聚焦也能出、读屏也念得到）。
    expect(button.attributes("title")).toBeUndefined();
    expect(button.attributes("aria-label")).toBeTruthy();

    // 提示内容只在触发时挂载，且 Portal 到 body —— 所以先聚焦再查 document.body。
    await button.trigger("focus");
    await flushPromises();
    const tip = document.body.querySelector('[data-slot="tooltip-content"]');
    expect(tip).not.toBeNull();
    expect(tip?.textContent).toContain("Ctrl+1");
  });
});
