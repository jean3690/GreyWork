...
describe("GroupedHistory · 会话状态标记", () => {
  /** 当前展开组（Alpha）里唯一的会话行状态。 */
  const statusAttr = (wrapper: ReturnType<typeof mountHistory>) => wrapper.get('[data-testid="session-status"]').attributes("data-status");

  it("没有消息 = 未开始", () => {
    const wrapper = mountHistory();
    expect(statusAttr(wrapper)).toBe("idle");
  });

  it("runningSessionId 命中且回合活跃 = 运行", async () => {
    const wrapper = mountHistory();
    const chat = useChatStore();
    chat.runningSessionId = "s-a1";
    chat.busy = true;
    await nextTick();
    expect(statusAttr(wrapper)).toBe("running");
    chat.busy = false;
  ...
  it("计划门挂起 = 等待中", async () => {
    ...
    session.getSession("s-a1")?.messages.push({
      id: "m1",
      role: "assistant",
      content: "",
      ts: Date.now(),
    ...
    await nextTick();
    expect(statusAttr(wrapper)).toBe("waiting");
  });

  it("有消息且空闲 = 结束", async () => {
    const wrapper = mountHistory();
    const session = useSessionStore();
    session.getSession("s-a1")?.messages.push({ id: "m2", role: "assistant", content: "好了", ts: Date.now() });
    await nextTick();
    expect(statusAttr(wrapper)).toBe("done");
  });
});
