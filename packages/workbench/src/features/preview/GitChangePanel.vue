...
/** 选中项（缓存键）的 diff 文件段。空文案（成功但无内容）→ 二进制/纯重命名。 */
const selectedDiff = computed(() => parseUnifiedDiff(git.selected === null ? "" : (git.diffCache[git.selected] ?? "")));
...
<template>
  <div class="flex min-h-0 min-w-0 flex-1 flex-col">
    ...
    <div class="flex shrink-0 items-center gap-2 border-b border-line px-3 py-1.5">
      ...
      <button
        v-if="git.stagedEntries.length > 0"
        type="button"
        data-testid="git-unstage-all"
        class="ms-auto shrink-0 cursor-pointer rounded-[6px] px-1.5 py-0.5 text-[10.5px] text-dim2 transition-colors hover:bg-panel hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-cyan disabled:cursor-not-allowed disabled:opacity-40"
      ...
      <button
        v-if="git.unstagedEntries.length > 0"
        type="button"
        ...
        :class="git.stagedEntries.length > 0 ? '' : 'ms-auto'"
        :disabled="git.staging"
        @click="git.stageAll()"
      >
      ...
      <button
        ...
        :class="git.stagedEntries.length + git.unstagedEntries.length > 0 ? '' : 'ms-auto'"
        :aria-label="t('preview.git.refresh')"
        @click="git.refresh()"
      >
