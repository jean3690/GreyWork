...
import McpJsonDialog from "@/features/settings/McpJsonDialog.vue";
import McpRegistryPane from "@/features/settings/McpRegistryPane.vue";
...
async function testMcpServer(entry: McpServerEntry): Promise<void> {
  // SSE 由 agent 建长连接，宿主不代为探活：按钮对它只标 aria-disabled（保持可聚焦，
  // 这样「为什么不能点」的提示才弹得出来），点击真正的兜底在这里。
  if (entry.transport === "sse") return;
...
function onJsonImport(entries: McpServerEntry[]): void {
  settings.replaceMcpServers(entries);
  mcpProbes.value = {};
  jsonOpen.value = false;
}

// ---------- 恢复默认 ----------
const resetTarget = ref(false);

function confirmReset(): void {
  if (!resetTarget.value) return;
  settings.resetMcpServers();
  mcpProbes.value = {};
...
<template>
  <div class="flex flex-col gap-4">
    ...
    <McpJsonDialog :open="jsonOpen" :servers="settings.mcpServers" @import="onJsonImport" @cancel="jsonOpen = false" />
    ...
    <ConfirmDialog
      v-if="resetTarget"
      title="恢复默认 MCP 配置？"
      message="将清除所有自定义服务器，仅保留内置示例：deepwiki（默认停用）。此操作不可撤销。"
      confirm-label="恢复默认"
      @confirm="confirmReset"
      @cancel="resetTarget = false"
    />
  </div>
</template>
