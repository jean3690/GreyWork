...
/** 工具类型 → 图标色调：让「改文件 / 跑命令 / 删东西」在行流里一眼可分。 */
...
/** 失败一律红，压过类型色调。 */
function rowTone(activity: ToolActivity): string {
  return activity.status === "failed" ? "text-destructive" : KIND_TONE[activity.kind];
}
...
/** 展示行：聚合行 + 该行改动规模（LCS 只算一次，别在模板里反复求）。 */
const displayRows = computed(() =>
  rows.value.map((row) => ({
    activity: row.activity,
    repeat: row.repeat,
    stat: diffStatOf(row.activity),
  })),
);
...
/** 运行进度条宽度（%）：进度条只在 running 时渲染。 */
const progressPct = computed(() => {
  if (!progress.value || progress.value.total === 0) return 0;
  return Math.round((progress.value.settled / progress.value.total) * 100);
});

/** 本条消息里的改动总量（+N/-N）；没有任何 diff 细节时为 null。 */
const changeStat = computed(() => aggregateDiffStat(props.activities));
...
<template>
  ...
  >
    ...
    >
      ...
      <span v-if="changeStat" class="flex flex-none items-center gap-1 font-mono text-[10px] tabular-nums" data-testid="tool-change-stat">
        <span v-if="changeStat.added" class="text-mint">+{{ changeStat.added }}</span>
        <span v-if="changeStat.removed" class="text-orange">-{{ changeStat.removed }}</span>
      </span>
      <span
    ...
    <!-- 运行进度条：2px，按已结算比例填充；结算后整条消失（不占视觉）。 -->
    <div v-if="lifecycle === 'running' && progress" class="h-0.5 w-full bg-amber/20" data-testid="tool-progressbar">
      <div class="h-full bg-amber transition-[width] duration-300" :style="{ width: `${progressPct}%` }" />
    </div>
      ...
      >
        ...
        >
          <Icon
            ...
            :size="11"
            class="flex-none"
            :class="[rowTone(row.activity), row.activity.status === 'in_progress' ? 'animate-pulse' : '']"
          />
          ...
          <span v-if="row.stat" class="flex flex-none items-center gap-1 font-mono text-[10px] tabular-nums">
            <span v-if="row.stat.added" class="text-mint" data-testid="tool-row-added">+{{ row.stat.added }}</span>
            <span v-if="row.stat.removed" class="text-orange" data-testid="tool-row-removed">-{{ row.stat.removed }}</span>
          </span>
