...
import ActivityBand from "@/features/activity/ActivityBand.vue";
 ...
 *
 * 「更多」菜单已改为 shadcn DropdownMenu，内容 Portal 到 body：wrapper.find 够不到，
 * 所以下面的菜单断言走 document.body。
 */

/** 菜单项（Portal 后落在 body 下）。 */
function menuItems(): DOMWrapper<Element>[] {
  return [...document.body.querySelectorAll('[data-testid="activity-overflow-item"]')].map((element) => new DOMWrapper(element));
}

function menuExists(): boolean {
  return document.body.querySelector('[data-testid="activity-overflow-menu"]') !== null;
}
...
describe("ActivityBand", () => {
  ...
  it("「更多」下拉列出 overflow 贡献，点选后激活并收起菜单", async () => {
    ...
    await flushPromises();
    expect(menuItems().map((item) => item.text())).toEqual(["tab.gamma"]);
  ...
  it("点条带外空白处收起「更多」菜单", async () => {
    ...
    // 点条带外空白处：reka 的 DismissableLayer 监听 document 上的 pointerdown，
    // 所以在 body 上派发即可命中。
