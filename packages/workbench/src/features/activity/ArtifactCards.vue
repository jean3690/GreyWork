  ...
  <div v-if="cards.length" class="flex flex-col gap-1.5">
    ...
    >
      <div class="flex items-center gap-2">
        ...
        <Hint :text="artifact.path ? `在预览面板打开 ${artifact.path}` : '该产物没有 VFS 路径，无法预览'" multiline>
          <button
            type="button"
            data-testid="artifact-name"
            :aria-disabled="!artifact.path || undefined"
        ...
        <Hint :text="artifact.path ? '在右侧预览面板打开' : '该产物没有 VFS 路径，无法预览'">
          <button
            type="button"
            data-testid="artifact-preview"
            :aria-disabled="!artifact.path || undefined"
            aria-label="预览"
            class="flex h-6 shrink-0 cursor-pointer items-center gap-1 rounded-full border border-line bg-panel px-2 text-[11px] text-dim transition-colors hover:border-line-2 hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-cyan aria-disabled:cursor-not-allowed aria-disabled:opacity-40"
            @click="openPreview(artifact)"
          >
        ...
        <Hint :text="artifact.diskPath ? '在系统文件管理器中打开所在文件夹' : '浏览器态未落盘：无磁盘路径可打开'">
          <button
            type="button"
            data-testid="artifact-reveal"
            :aria-disabled="!artifact.diskPath || undefined"
            aria-label="在文件夹中打开"
            class="flex h-6 shrink-0 cursor-pointer items-center gap-1 rounded-full border border-line bg-panel px-2 text-[11px] text-dim transition-colors hover:border-line-2 hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-cyan aria-disabled:cursor-not-allowed aria-disabled:opacity-40"
